...
DataVault is an original, server-only Roblox profile persistence module inspired by the safety goals of ProfileService, ProfileStore, and Suphi's DataStore Module. It gives each key one active writer, keeps data cached in memory, autosaves it, and releases the lock when the session ends.
...
Copy [`src/DataVault.luau`](src/DataVault.luau) into a `ModuleScript` named `DataVault` under `ServerScriptService`, or sync the included Rojo project.

DataVault must never be required from a `LocalScript`. In Studio, use `UseMock = true` for safe tests. Only enable Studio API access against a separate test universe; Roblox warns that Studio can otherwise access the same stores as production.
...
The module binds its own shutdown callback and releases all active profiles. You do not need a second `BindToClose` handler for DataVault.

## API

### `DataVault.new(name, template, options?) -> Store`
...
`LoadAsync` accepts `Timeout`, a `Cancel` callback, and an `OnLocked(sessionInfo)` callback. `OnLocked` may return `"Cancel"`; returning `"Wait"` or nothing continues waiting. DataVault intentionally has no immediate force-steal switch. A live owner must release, or its heartbeat must become stale, before a different server can own the key.

`Cancel`, `OnLocked`, and migration callbacks must not yield. This keeps shutdown and lock handling bounded; DataVault treats a yielding callback as an error.
...
```luau
DataVault.OnError:Connect(function(message, storeName, key, operation)
	warn(message, storeName, key, operation)
end)

DataVault.OnCorruption:Connect(function(storeName, key, message)
	-- Alert your telemetry. DataVault refuses to overwrite the malformed value.
end)

DataVault.OnCriticalStateChanged:Connect(function(isCritical)
...
Migration functions must be deterministic and must not yield. DataVault rejects a yielding migration and safely releases the claimed session without replacing the stored data.

```luau
local Store = DataVault.new("PlayerData_v1", TEMPLATE_V3, {
	SchemaVersion = 3,
	Migrations = {
		[2] = function(data)
...
Keep metadata small: Roblox currently permits 50 bytes for a metadata key, 250 for a value, and 300 total. DataVault conservatively verifies the JSON-encoded metadata table against 300 bytes.
...
- DataVault is optimized for one long-lived owner per key, especially player profiles. It is not an OrderedDataStore leaderboard or a high-frequency cross-server database.
...
[`tests/DataVault.manual.server.luau`](tests/DataVault.manual.server.luau) is a dependency-free Studio regression script. Sync [`test.project.json`](test.project.json), start a server, and check for `DataVault manual tests passed`.
